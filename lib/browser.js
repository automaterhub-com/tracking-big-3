...
async function createTrackingSession(carrierName, options = {}) {
  ...
  // When DISPLAY is set (Xvfb in Docker), always use headed mode to bypass Cloudflare
  ...
  if (forceHeaded || hasDisplay) {
    if (hasDisplay) console.log(`[browser] DISPLAY=${process.env.DISPLAY} — using headed mode (Xvfb)`);
